...
function CleverTabs(self, options) {
    ...
    this.options = $.extend({
        ...
        //Tab用于显示的Panel模板
        tabPanelTemplate: '<div id="cleverTabPanelItem-#{id}" style="height:100%"><iframe frameBorder="0" scrolling="yes" style="width: 100%; display: inline; height: 100%;" src="#{src}"></iframe></div>',
        //Tab唯一id生成器
    ...
    if (this instanceof CleverTabs) {
        CleverTabs.prototype.resizePanelContainer = function () {
            if (pc.attr('id').indexOf('cleverTabsPanelContainer') == 0) {
            	var height = $(window).height()-$("#north").height()-47;
                pc.css('height', height);
            }
        };
    }
};
...
//    alert(options.url);
    ...
    .replace(/#\{label\}/g, function () {
        //如果Tab的Label大于8个字符，则强制使其为前8个字符加'...'
        if (options.label.length > 8) {
            return options.label.substring(0, 8) + '...';
        }
    ...
    tabHeader.bind("click",function() {
    	//自动刷新测评报告导入流程界面
    	if(tab.url.indexOf('cpdljk.xhtml') != -1){
    		tab.refresh();
    	}
    });
    ...
    if (self.panelContainer.attr('id').indexOf('cleverTabsPanelContainer') === 0) {
    	var height = $(window).height()-116;
    	 self.panelContainer.css('height', height);
    	$("#toolbar-pane").resize(function(){
    		var north_height = $("#north").height();
			if(north_height!=0){
				height = $(window).height()-$("#toolbar-pane").height()-43;
				//height = $("#navigator-pane").height();
				self.panelContainer.css('height', height);
			}
		});
    	
    	$("#north").resize(function(){
    		var north_height1 = $("#north").height();
			if(north_height1==0){
...
/*CleverTabs.prototype.setupContextMenu = function () {
    ...
    if (!self.options.contextMenu) {
        contextMenu = {
            element: $('<ul id="cleverTabsContextMenu" style="margin-left:-200px;margin-top:-80px;">'
                    ...
                    + '<li id="mnuRefresh" ><a href="#" onclick="tab.refresh()"><span class="ui-icon ui-icon-refresh" style="float: left; margin-right: 5px;"></span>重新载入</a></li><br/>'
                    + '<li id="mnuCloseAll" style="margin-top:-10px;"><a href="#" onclick="tabs.clear()"><span class="ui-icon ui-icon-closethick" style="float: left; margin-right: 5px;margin-top:0px;"></span>关闭所有页面</a></li>'
        + '</ul>'),
...
CleverTabs.prototype.clear = function () {
    ...
    var hasLock = self.element.find('span.ui-icon-locked').size() > -1;
...
CleverTab.prototype.setLock = function (lock, changeOrgLock) {
    ...
    if (!lock) {
        ...
        var btnClose = $('<a href="javascript:void(0)" class="ui-corner-all" title="关闭" style="position: absolute; cursor: pointer; padding: 0px; top: 1px; right: 1px"><span class="ui-icon ui-icon-close"></span></a>');
