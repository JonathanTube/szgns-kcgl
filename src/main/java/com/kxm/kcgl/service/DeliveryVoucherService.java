...
public class DeliveryVoucherService {
	...
	private class MapKey {
		...
		public MapKey(String brandName, String productName, String techName, String thicknessName,
				int amount) {
			super();
		...
		public boolean equals(Object obj) {
			...
			return mapKey.brandName == brandName && mapKey.productName == productName
					&& mapKey.techName == techName && mapKey.thicknessName == thicknessName;
		}
	...
	public Map<String, Object> query(Date date) {
		List<DeliveryVoucherBean> list = deliveryVoucherDao.query(date);
		// 获取当前月份一共有多少天
		Calendar now = Calendar.getInstance();
		int days = getDaysByYearMonth(now.get(Calendar.YEAR), now.get(Calendar.MONTH) + 1);
		Map<MapKey, int[]> map = new HashMap<DeliveryVoucherService.MapKey, int[]>();
		for (DeliveryVoucherBean w : list) {
			MapKey mk = new MapKey(w.getBrandName(), w.getProductName(), w.getTechName(),
					w.getThicknessName(), w.getAmount());
			if (map.containsKey(w)) {
				int[] vals = map.get(w);
				String[] d = w.getCreateTime().split("-");
				String _d = d[2];
				if (_d.startsWith("0")) {
					_d = _d.substring(1);
				}
	...
	public List<DeliveryVoucherBean> queryDetail(Date date,String custId) {
		List<DeliveryVoucherBean> list = deliveryVoucherDao.queryByDate(date,custId);
		return list;
	}
